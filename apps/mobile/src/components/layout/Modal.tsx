	...
	return (
		<>
			...
			>
				{/* Title */}
				{props.title && (
					<Text style={tw`text-center text-base font-medium text-ink`}>{props.title}</Text>
				)}
				<View style={tw`mt-4 px-6`}>
					{/* Description */}
					{props.description && <Text style={tw`text-sm text-ink-dull`}>{props.description}</Text>}
					{/* Children */}
					...
					<View style={tw`flex flex-row pt-5`}>
						...
						{props.ctaAction && (
							...
							>
								<Text style={tw`text-sm font-medium text-ink`}>{props.ctaLabel}</Text>
							</Button>
						)}
					</View>
				</View>
